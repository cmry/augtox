...
def _enum_stem_match(
    ...
    """
    stemmed_enum_list1 = [
        (word_pair[0], stemmer.stem(word_pair[1]))
    ...
    enum_unmat_hypo_list = (
        list(zip(*enum_unmat_hypo_list)) if
        len(enum_unmat_hypo_list) > 0 else []
    )
    ...
    enum_hypothesis_list = list(
        filter(lambda x: x[0] not in enum_unmat_hypo_list,
...
def stem_match(hypothesis, reference, stemmer=PorterStemmer()):
    ...
    enum_hypothesis_list, enum_reference_list = \
        _generate_enums(hypothesis, reference)
...
def _enum_wordnetsyn_match(enum_hypothesis_list, enum_reference_list,
    ...
    :param enum_hypothesis_list: enumerated hypothesis list
    :param enum_reference_list: enumerated reference list
    :param wordnet: a wordnet corpus reader object (default
                    nltk.corpus.wordnet)
...
def _enum_allign_words(
    ...
    return (
        sorted(
            exact_matches + stem_matches + wns_matches,
            key=lambda wordpair: wordpair[0]
        ),
        enum_hypothesis_list,
        enum_reference_list,
    )
...
def meteor_score(
    ...
    >>> reference1 = 'It is a guide to action that ensures that the military
                      will forever heed Party commands'
    >>> reference2 = 'It is the guiding principle which guarantees the
    ...
    >>> round(meteor_score([reference1, reference2, reference3], hypothesis1),
              4)
    0.7398
        If there is no words match during the alignment the method returns the
        score as 0. We can safely  return a zero instead of raising a
